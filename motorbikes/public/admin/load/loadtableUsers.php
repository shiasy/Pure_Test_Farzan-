...
if ($_SESSION[user][id]==''&&$_COOKIE[userID]==''){
	...
	switch ($data[operator]) {case 'list':
			$number=Users::get(array('betcreated_at1'=>$data[ofdate],'betcreated_at2'=>$data[todate],'likeemail'=>$data[email],'likename'=>$data[name],'typeShow'=>1))[0][id];
            $numberall=$number;
			...
			$req=Users::get(array('betcreated_at1'=>$data[ofdate],'betcreated_at2'=>$data[todate],'likeemail'=>$data[email],'likename'=>$data[name],"limit"=>$valPage,"firstLimit"=>$valPageNumber));
			...
			foreach ($req as $key => $value) {
					...
					<tr>
						...
						<td class="ltr"><?=Views::timeToStr($value[created_at])?></td>
						<td class="ltr"><?=Views::timeToStr($value[updated_at])?></td><td>
						    ...
						    <div class="dropdown-menu px-2">
						    	...
						    	<button  data-toggle="modal" data-target="#editmodal" type="button" class="btn btn-info w-100 my-1" onclick='popup(`{"operator":"change","type":"show","id":"<?= Used::jwt_encode($value[id])?>","dir":"<?= $data[dir]?>"}`,"pop/popUsers","#editmodal .modal-content",``);'>مشاهده و ویرایش</button>
								

								<h5 class="dropdown-header">حذف</h5>
								
								<button type="button" class="btn btn-danger w-100 my-1" onclick='if (confirm("آیا از درخواست خود مطمئن هستید؟") == true) {popup(`{"operator":"delete","id":"<?= Used::jwt_encode($value[id])?>"}`,"pop/popUsers",".body #checked",``);}'>حذف</button>
